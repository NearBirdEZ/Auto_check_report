...
class API_Naumen:
    ...
    def __start_naumen(self):
        ...
        self.enter_words("password", self.password, '//*[@id="LogonForm"]/table/tbody/tr/td/table/tbody/tr[4]/td/input')
    ...
    def description_body(self, request):
        ...
        self.search_by_request(request)
        time.sleep(1)
        descriprion = self.driver.find_element_by_class_name("servicecall_description_inner").text
        return descriprion
    ...
    def send_mail(self, request, text, description):
        ...
        link_mail = \
            self.driver.find_element_by_id("ServiceCall.MailingList.SCMailing").get_attribute("href").split("?")[-1]
        self.driver.get("http://sdn.pilot.ru:8080/fx/$guic/ru.naumen.guic.components.forms.form_jsp?" + link_mail)
        self.driver.find_element_by_name('mailText').clear()
        text = f"""{text} {request}\n{description}
                    \n\nС уважением,\nСлужба Поддержки Пользователей\nООО \"Пилот - бизнес решения для 
торговли\"\n107023, Москва, 
                    Барабанный переулок, дом 3\nтел. +7 495 564-8794\nфакс +7 495 564-8369\ne-mail: service_desk@pilot.ru """
        self.driver.find_element_by_id("mailText").send_keys(text)
        time.sleep(1)
    ...
    def send_comments(self, request, comment_text):
        ...
        self.driver.find_element_by_id("text").send_keys(comment_text)
